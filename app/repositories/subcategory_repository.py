...
class SubCategoryRepository:
    ...
    # [ENTRADA: subcategory_data - dados da subcategoria via schema, category_internal_id - ID interno da categoria]
    # [SAIDA: SubCategory - instância da subcategoria criada]
    # [DEPENDENCIAS: SubCategory, self.db]
    def create(self, subcategory_data: SubCategoryCreate, category_internal_id: int) -> SubCategory:
        ...
        db_subcategory = self.db.query(SubCategory).options(joinedload(SubCategory.category)).filter(SubCategory.id == db_subcategory.id).first()
        return db_subcategory

    # [GET BY PUBLIC ID]
    # [Busca uma subcategoria pelo UUID público]
    # [ENTRADA: public_id - UUID público da subcategoria]
    # [SAIDA: Optional[SubCategory] - subcategoria encontrada ou None]
    # [DEPENDENCIAS: SubCategory, self.db]
    def get_by_public_id(self, public_id: UUID) -> Optional[SubCategory]:
        return self.db.query(SubCategory).options(joinedload(SubCategory.category)).filter(SubCategory.public_id == public_id).first()

    # [GET BY NAME]
    # [Busca uma subcategoria pelo nome]
    # [ENTRADA: name - nome da subcategoria]
    # [SAIDA: Optional[SubCategory] - subcategoria encontrada ou None]
    # [DEPENDENCIAS: SubCategory, self.db]
    def get_by_name(self, name: str) -> Optional[SubCategory]:
        return self.db.query(SubCategory).options(joinedload(SubCategory.category)).filter(SubCategory.name == name).first()

    # [GET BY CATEGORY]
    # [Busca subcategorias por categoria]
    # [ENTRADA: category_id - ID interno da categoria, skip - registros a pular, limit - limite]
    # [SAIDA: List[SubCategory] - lista de subcategorias da categoria]
    # [DEPENDENCIAS: SubCategory, self.db]
    def get_by_category(self, category_id: int, skip: int = 0, limit: int = 100) -> List[SubCategory]:
        return self.db.query(SubCategory).options(joinedload(SubCategory.category)).filter(
    ...
    # [SAIDA: List[SubCategory] - lista de subcategorias]
    # [DEPENDENCIAS: SubCategory, self.db]
    def get_all(self, skip: int = 0, limit: int = 100) -> List[SubCategory]:
        return self.db.query(SubCategory).options(joinedload(SubCategory.category)).offset(skip).limit(limit).all()

    # [GET TOTAL COUNT]
    # [Conta total de subcategorias ativas]
    # [ENTRADA: nenhuma]
    # [SAIDA: int - número total de subcategorias]
    # [DEPENDENCIAS: SubCategory, self.db]
    def get_total_count(self) -> int:
        return self.db.query(SubCategory).count()

    # [GET TOTAL COUNT BY CATEGORY]
    # [Conta total de subcategorias ativas por categoria]
    # [ENTRADA: category_id - ID interno da categoria]
    # [SAIDA: int - número total de subcategorias da categoria]
    # [DEPENDENCIAS: SubCategory, self.db]
    def get_total_count_by_category(self, category_id: int) -> int:
    ...
    # [SAIDA: SubCategory - subcategoria atualizada]
    ...
    def update(self, subcategory: SubCategory, subcategory_data: SubCategoryUpdate, category_internal_id: Optional[int] = None) -> SubCategory:
        ...
        subcategory = self.db.query(SubCategory).options(joinedload(SubCategory.category)).filter(SubCategory.id == subcategory.id).first()
        return subcategory
